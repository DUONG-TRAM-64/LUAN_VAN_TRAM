...
def preprocess_data(train_df, test_df, model_num):
    train_df, val_df = train_test_split(train_df, test_size=0.125, random_state=1112)
...
def train_model(model, X_train, y_train, X_val, y_val, class_weights):
    ...
    callbacks = [
        EarlyStopping(monitor='val_loss', patience=999, restore_best_weights=True, verbose=1),
        ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=999, min_lr=1e-7, verbose=1)
    ]
    start = time.time()
